...
static_assert(std::is_nothrow_move_constructible<csv_error_info>::value, "");
static_assert(std::is_nothrow_copy_constructible<csv_error_info>::value, "");
static_assert(std::is_nothrow_move_assignable<csv_error_info>::value, "");
static_assert(std::is_nothrow_copy_assignable<csv_error>::value, "");
...
TEST(TestCsvError, Info)
{
    csv_error e("Some error occurred");
    {
        std::stringstream s;
        s << e.info();
    ...
    e.set_physical_position(csv_error::npos - 1, csv_error::npos - 2);
    std::string is;
    {
        std::stringstream s;
        s << e.info();

...
{
    using target_type = std::basic_string<Ch, Tr>;

    template <class Sink>
    string_field_translator<std::decay_t<Sink>, Ch, Tr> operator()(Sink&& sink)
        const /* TODO: noexcept? */
    {
        return string_field_translator<std::decay_t<Sink>, Ch, Tr>(
            std::forward<Sink>(sink));
    }
};

...
{
    using type = typename
        detail::record_xlate::default_field_translator_factory_impl<T>::type;
};
...
public:
        ...
        basic_table_scanner<Ch, Tr, Allocator>& scanner)
    {
        ...
        for (const auto& e : m_) {
            ...
            // "skipped"
            std::move(*e.second)(static_cast<std::size_t>(-1), scanner);
        }
        return false;
    }
};
...
make_record_translator(FR&& f, FieldSpecRs&&... specs)
{
    ...
    return make_record_translator(std::allocator_arg, std::move(a),
        std::forward<FR>(f), std::forward<FieldSpecRs>(specs)...);
}
